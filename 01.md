...
### Synced folders
Sometimes we'd like to have the local files automatically sync with the VM. While Virtual box provides for a 'shared folder' the approach is a little slow. Vagrant provides a faster alternative.

* folders can be synced using the `config.vm.synced_folder` method in the VagrantFile

Ooh! by the way, did I tell you that it's a good idea to pay attention to capitalization? You never know when case-sensitivity issues may cause unnecessary confusion. :) Onwards now.

* Edit the VagrantFile again to make it look like so:
...
for Day 2: using a docker provider with Vagrant.
